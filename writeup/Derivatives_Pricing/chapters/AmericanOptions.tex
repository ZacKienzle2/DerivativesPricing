...
\begin{proof}
	...
	\]
	This process is an exponential martingale (see \Cref{thm:ExponentialMartingale}) under $\widetilde{\Prob}$ (its differential has a $d\widetilde{W}(t)$ term and no $dt$ term). According to \Cref{thm:DoobsStoppingTheoremForContinuousMartingales}, the stopped martingale
	\[
...
\begin{figure}[H]
	\centering
	\input{plotting/ExerciseBoundaryValue.pgf}
	\caption{\((K-L)(\frac{x}{L})^{-\frac{2r}{\sigma^2}}\) for three values of \(L\). }
	\label{fig:ExerciseBoundaryValue}
\end{figure}
...
\begin{figure}[H]
	\centering
	\input{plotting/GraphOfGL.pgf}
	\caption{Graph of \(g(L)\).}
	\label{fig:GraphOfGL}
\end{figure}
...
\begin{proof}
	...
	\[
		e^{-r(t\land\tau_{L_*})}v_{L_*}(S(t \land \tau_{L_*})) = v_{L_*}(0) + \int_0^{t\land\tau_{L_*}} e^{-ru}\sigma S(u)v'_{L_*}(S(u)) \, d\widetilde{W}(u).
	\]
...
\begin{figure}[H]
	\centering
	\input{plotting/ConvexFunctionPayoff.pgf}
	\caption{The convex function \(h(x) = (x-K)^+\).}
	\label{fig:ConvexFunctionPayoff}
\end{figure}
...
We assume there are times $0 < t_1 < t_2 < \dots < t_n < T$, and at each time $t_j$ the dividend paid is $a_j S(t_j-)$, where $S(t_j-)$ denotes the asset price just prior to the dividend payment. The asset price $S(t_j)$ after the dividend payment is the asset price before the dividend payment less the dividend payment:
\begin{equation}
...
\begin{equation}
	c_{n-1}(t_n, x) = h_n(t_n, x), \quad x \ge 0.
