...
\Levy{} processes serve as the fundamental building blocks of continuous-time stochastic modelling, representing the continuous analogue of random walks through their defining property of stationary and independent increments \parencite{sato_levy_2013, schilling_introduction_2016}.

Analytically, their distributional structure is isomorphic to the class of infinitely divisible distributions via the \Levy{}-Khintchine formula, which characterizes the characteristic exponent through a unique triplet comprising a deterministic drift, a Gaussian variance, and a \Levy{} measure governing jump intensity \parencite{applebaum_levy_2013}.

From a pathwise perspective, the \Levy{}-\Ito{} decomposition explicitly resolves the trajectory into the superposition of a deterministic trend, a continuous Gaussian martingale, and a discontinuous jump component driven by a Poisson random measure \parencite{kyprianou_fluctuations_2014}.

Within derivatives pricing, this framework generalizes geometric Brownian motion to endogenously capture the leptokurtosis and skewness inherent in asset returns, thereby rectifying the distributional inadequacies of diffusion-only models \parencite{cont_financial_2004}.

\subsection{From Random Walks to \Levy{} Processes}

\begin{defn}{\Levy{} process}{LevyProcess}
	A cadlag\footnotemark{} stochastic process \((X_t)_{t \ge 0}\) on \((\Omega, \cF, \Prob)\) with values in \(\R^d\) such that \(X_0 = 0\) is called a \Levy{} process if it possesses the following properties:
	\begin{enumerate}[label=\arabic*.]
		\item \emph{Independent increments}: for every increasing sequence of times \(t_0 \dots t_n\), the random variables \(X_{t_0}, X_{t_1} - X_{t_0}, \dots, X_{t_n} - X_{t_{n-1}}\) are independent.
		\item \emph{Stationary increments}: the law of \(X_{t+h} - X_t\) does not depend on \(t\).
		\item \emph{Stochastic continuity}: \(\forall \varepsilon > 0, \lim_{h \to 0} \Prob(\abs{X_{t+h} - X_t} \ge \varepsilon) = 0\).
	\end{enumerate}
\end{defn}

\footnotetext{Some authors do not impose the cadlag (right-continuity and left limits) property in the definition of a \Levy{} process, but it can be shown that every \Levy{} process (defined without the cadlag property) has a unique modification that is cadlag; therefore, the cadlag property can be assumed without loss of generality.}
...
\section{Building Levy Processes}

\subsection{Model building with \Levy{} Processes}
Let us start by some general considerations on \Levy{} processes, their use as models for price dynamics and different ways to specify such models.
\section{Simulating Levy Processes}



\section{Stochastic Calculus for Jump Processes}

Consider now a jump-diffusion process
\begin{equation} \label{eq:JumpDiffusionProcess}
	X_t = \sigma W_t + \mu t + J_t = X^c(t) + J_t,
\end{equation}
where \(J\) is a compound Poisson process and \(X^c\) is the continuous part of \(X\):
\begin{equation} \label{eq:ComponentDefinitions}
...
\begin{equation} \label{eq:FinalItoFormula}
	\begin{split}
...
\begin{xattention}{}
	...
	\begin{equation} \label{eq:EquivalentItoExpression}
		\begin{split}
			f(X_t) - f(X_0) & = \int_0^t f'(X_{s-}) \di X_s + \int_0^t \frac{\sigma^2}{2} f''(X_s) \di s                                    \\
			                & \quad + \sum_{0 \le s \le t, \Delta X_s \neq 0} [f(X_{s-} + \Delta X_s) - f(X_{s-}) - \Delta X_s f'(X_{s-})].
		\end{split}
	\end{equation}
	When the number of jumps is finite, this form is equivalent to \cref{eq:FinalItoFormula}. However, as we will see below, the form \cref{EquivalentItoExpression} is more general, wherein both the stochastic integral and the sum over the jumps in \cref{eq:EquivalentItoExpression} are well-defined for any semi-martingale, even if the number of jumps is infinite, while the sum in \cref{eq:FinalItoFormula} may not converge if jumps have infinite variation.
...
\subsection{Risk-neutral modelling with exponential \Levy{} Processes}
