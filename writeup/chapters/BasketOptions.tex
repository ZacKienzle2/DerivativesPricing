...
We may rewrite \Cref{eq:BasketCallPayoff} as follows\footnote{This representation is introduced because it appears to give a better approximation of the price of a basket option than formula \Cref{eq:BasketCallPayoff}.}
...
\end{equation}
In view of \Cref{eq:StockDynamicsVector}, we have (recall that \(F_{S^i}(0, T) = e^{rT} S_0^i\))
\[
...
More precisely, the random variable \(\eta_T\) in \Cref{eq:GeometricAverage} has Gaussian distribution with expected value \(0\) and variance
\begin{align*}
...
\]
We conclude that \(\hat{K} = \tilde{K} + c - 1\). The expectation in \Cref{eq:ApproxBasketPrice} can now be evaluated explicitly.
