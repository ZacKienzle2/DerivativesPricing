...
\Cref{thm:UsefulMatrixRules} summarises a number of useful matrix rules for the concepts that we have discussed so far.
...
\begin{proof}
	By Property 1 of \Cref{defn:MoorePenrosePseudoInverse} we have \(\vecb{PU} = \vecb{U U}^+ \vecb{U} = \vecb{U}\), so that \(\vecb{P}\) projects any vector in \(\mathcal{V}\) onto itself. Moreover, \(\vecb{P}\) projects any vector in \(\mathcal{V}^\perp\) onto the zero vector.
\end{proof}
